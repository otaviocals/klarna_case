...
with DAG(
    ...
    deploy_model = DeployModelOperator(
        ...
        new_version_location="{{ ti.xcom_pull(task_ids='test_training', key='return_value')['Training']['ModelArtifacts']['S3ModelArtifacts'].strip('\\\"').strip('s3://klarna-case-model-bucket/') }}",
